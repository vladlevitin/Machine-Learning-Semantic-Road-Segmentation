...
def extract_data(filename, num_images):
    ...
    for i in range(0, num_images):
        ...
        image_filename = filename + files[i]
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    test_data_filename = 'Datasets/test_set_images/test_' #add the index later 
    ...
    def get_prediction(img):
        data = numpy.asarray(img_crop(img, IMG_PATCH_SIZE_TEST, IMG_PATCH_SIZE_TEST))
        data_node = tf.constant(data)
        output = tf.nn.softmax(model(data_node))
        output_prediction = s.run(output)
        img_prediction = label_to_img(img.shape[0], img.shape[1], IMG_PATCH_SIZE_TEST, IMG_PATCH_SIZE_TEST, output_prediction)
    ...
    def get_prediction_with_groundtruth(filename, image_idx):
        ...
        #np.str(i) + "/" + "test_" + np.str(i)
        image_filename = filename + str(image_idx)+"/" + "test_"+str(image_idx) + ".png"
        img = mpimg.imread(image_filename)
    ...
    with tf.Session() as s:
        ...
        for i in range(1, 50 + 1):
            ...
            plt.imsave(prediction_training_dir + "prediction_" + str(i) + ".png", pimg, cmap=cm.gray)
